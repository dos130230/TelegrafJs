...
const keyboard5 = [
    ...
    Keyboard.make(['↩️ Назад', '🏠 Главная', '❌ Отмена'], { columns: 2, }).reply(),
...
const keyboard6 = [
    [
        Keyboard.make([Key.callback('Shoshilinch', 'action1_uz')]).inline(),
        Keyboard.make([Key.callback('Standart', 'action2_uz')]).inline(),
        Keyboard.make([Key.callback('📝 Xaydovchi haqida', 'action3_uz')]).inline()
    ],
    [
        Keyboard.make([Key.callback('Торопиться', 'action1_ru')]).inline(),
        Keyboard.make([Key.callback('Стандарт ', 'action2_ru')]).inline(),
        Keyboard.make([Key.callback('📝 О водителе', 'action3_ru')]).inline()
    ]
]
